...
> **Note:** The `aec` virtual ALSA device defined in `configs/alsa-aec.conf` is **not compatible** with the default WebRTC service:
>
> - The default service runs in raw loopback-router mode (writes directly to the speaker via `-p plughw:wm8960soundcard,0`)
> - The `aec` device routes playback to BOTH the speaker AND the loopback simultaneously
> - Using `aplay -D aec` with the default service would cause duplicate playback
> - The `aec` device is for advanced users running the EC binary without the `-p` flag
>
> Stick to `hw:Loopback,0,0` / `hw:Loopback,1,1` unless you know what you're doing.
